...
class NewUserForm(UserCreationForm):
    ...
    usermode = forms.BooleanField(required=False, label='Personal Trainer')

    class Meta:
        model = User
        fields = ("username", "first_name", "last_name", "email", "password1","password2","usermode")
    ...
    def save(self, commit=True):
        ...
        user.usermode= self.cleaned_data['usermode']
        if commit:
            user.save()
        return user
